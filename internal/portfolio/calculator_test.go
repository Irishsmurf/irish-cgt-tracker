...
func TestSettleSale_MultiVestFIFO(t *testing.T) {
	db, mock, err := sqlmock.New(sqlmock.QueryMatcherOption(sqlmock.QueryMatcherEqual))
	if err != nil {
		t.Fatalf("an error '%s' was not expected when opening a stub database connection", err)
	}
	...
	saleID := uuid.New().String()
	vestID1 := uuid.New().String()
	vestID2 := uuid.New().String()
	...
	mock.ExpectQuery("SELECT v.id, v.date, v.symbol, v.quantity, v.strike_price_cents, v.ecb_rate, COALESCE(SUM(sl.quantity), 0) as used_qty FROM vests v LEFT JOIN sale_lots sl ON v.id = sl.vest_id GROUP BY v.id ORDER BY v.date ASC").
		WillReturnRows(sqlmock.NewRows([]string{"id", "date", "symbol", "quantity", "strike_price_cents", "ecb_rate", "used_qty"}).
			AddRow(vest1.ID, vest1.Date, vest1.Symbol, vest1.Quantity, vest1.StrikePriceCents, vest1.ECBRate, 0).
			AddRow(vest2.ID, vest2.Date, vest2.Symbol, vest2.Quantity, vest2.StrikePriceCents, vest2.ECBRate, 0))

	// 3. Expect the saveLot inserts
	mock.ExpectExec("INSERT INTO sale_lots (sale_id, vest_id, quantity) VALUES (?, ?, ?)").
		WithArgs(saleID, vestID1, int64(100)).
		WillReturnResult(sqlmock.NewResult(1, 1))
	mock.ExpectExec("INSERT INTO sale_lots (sale_id, vest_id, quantity) VALUES (?, ?, ?)").
		WithArgs(saleID, vestID2, int64(50)).
		WillReturnResult(sqlmock.NewResult(1, 1))
...
func TestSettleSale_AlreadySettled(t *testing.T) {
	db, mock, err := sqlmock.New(sqlmock.QueryMatcherOption(sqlmock.QueryMatcherEqual))
	if err != nil {
		t.Fatalf("an error '%s' was not expected when opening a stub database connection", err)
	}
	...
	err = s.SettleSale(saleID)
	if err == nil {
		t.Error("expected an error for already settled sale, but got nil")
	}
